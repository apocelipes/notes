...
遗憾的是这样的代码会收获编译错误：`msg.Encode undefined (type *T is pointer to type parameter, not type parameter)`。这是个常见错误了，直接取泛型变量的指针大多数时候都会报这种错，我以前的[博客](./golang泛型的糙点.md)里有解释过原因，这里不再赘述。
