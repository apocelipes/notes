...
4. 在程序开始执行前或者第一次使用多版本函数前（取决于环境变量`LD_BIND_NOW`的值），`IFUNC`会被调用，它会返回最合适的函数实现
...
上面是编译器自动生成时的例子，手动编写时的步骤是类似的，具体可以参考glibc源码路径下的`sysdeps/x86_64/multiarch/`目录里的代码。
...
属性`__attribute__((ifunc ("resolve_foo")));`指定间接函数调用的转发函数，剩下的编译器会处理。
